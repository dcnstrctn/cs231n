...
def batchnorm_backward(dout, cache):
  ...
  dmean = np.sum(-1/np.sqrt(sample_var + eps) * dx_hat, axis = 0) -2.0*np.sum((x-sample_mean), axis = 0)/N*dvar
  
  dx = 1/np.sqrt(sample_var + eps) * dx_hat
  dx += dmean/N
  dx += dvar/N*2.0*(x - sample_mean)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for i in range(H_out):
      for j in range(W_out):
          for k in range(F):
              out[:, k, i, j] = np.sum(
                      x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]*w[k, :, :, :], 
                       axis = (1, 2, 3)) +b[k]
...
def conv_backward_naive(dout, cache):
  ...
  for n in range(N):
      for k in range(F):
          for i in range(H_out):
              for j in range(W_out):
                  dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += dout[n, k, i, j]*w[k, :, :, :]
                  dw[k, :, :, :] += dout[n, k, i, j]*x_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(H_out):
      for j in range(W_out):
          x_masked = x[:, :, i*s:i*s+h, j*s:j*s+w]
          max_x_masked = np.max(x_masked,axis=(2,3))[:,:,None,None]
          mask = (x_masked == max_x_masked)
