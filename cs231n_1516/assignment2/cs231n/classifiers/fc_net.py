...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i, dim in enumerate(hidden_dims):
        self.params['W%d'%(i+1)] = weight_scale * np.random.randn(in_dim, dim)
  ...
  def loss(self, X, y=None):
    ...
    dout, dw, db = affine_backward(dscores, out_cache)
    grads['W%d'%(self.num_layers)] = dw + self.reg*self.params['W%d'%(self.num_layers)]
    grads['b%d'%(self.num_layers)] = db
    
    for idx in range(self.num_layers - 1):
        i = self.num_layers - 2 - idx
        loss += 0.5*self.reg*np.sum(self.params['W%d'%(i+1)]**2)
        if self.use_dropout:
            dout = dropout_backward(dout, dp_cache[i])
        if self.use_batchnorm:
            dout, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, fc_mix_cache[i])
            grads['gamma%d'%(i+1)] = dgamma
            grads['beta%d'%(i+1)] = dbeta
        else:
            dout, dw, db = affine_relu_backward(dout, fc_mix_cache[i])
